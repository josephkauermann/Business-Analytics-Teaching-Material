...
# 5. Create a direct download link for the web browser
from IPython.display import HTML
import base64

csv_raw = rpt_final_df.to_csv(index=False)
b64 = base64.b64encode(csv_raw.encode()).decode()
html = f'<a download="rpt_1_playground_data.csv" href="data:text/csv;base64,{b64}" target="_blank"><b>⬇️ Click here to download the file to your computer!</b></a>'
...
1. Follow the **RPT-1 Setup Guide in your Setup PDF** to upload our dataset to the RPT-1 playground.
...
# 1. Sort customers by predicted CLV and select the Top 100 to call
top_100 = results.sort_values(by='___', ascending=False).head(___)

# 2. Calculate the PREDICTED average CLV (What we pitch to management)
avg_predicted_clv = top_100['___'].mean()

# 3. Calculate the ACTUAL average CLV (What the company actually earned in reality)
avg_actual_clv = top_100['___'].mean()

# 4. Apply the business formula (100 calls * 3% churn * 20% success rate)
churn_rate = 0.03  
success_rate = 0.20
multiplier = 100 * churn_rate * success_rate

projected_revenue = multiplier * avg_predicted_clv
actual_revenue = multiplier * avg_actual_clv
random_revenue = 36000 # Baseline from Exercise 2

# 5. Print the results
print("-" * 55)
print("BUSINESS IMPACT ANALYSIS")
print("-" * 55)
print(f"1. Random Baseline Strategy:   ${random_revenue:,.0f}")
print("")
print(f"2. ML Projected Revenue:       ${projected_revenue:,.0f}")
print(f"   Projected Improvement:      +{((projected_revenue/random_revenue)-1)*100:,.0f}%")
print("")
print(f"3. ML Actual Realized Revenue: ${actual_revenue:,.0f}")
print(f"   Actual Improvement:         +{((actual_revenue/random_revenue)-1)*100:,.0f}%")
print("-" * 55)
